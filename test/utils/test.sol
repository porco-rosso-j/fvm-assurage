...
contract TestUtils is DSTest {
    ...
    bytes constant ZERO_DIVISION =
        abi.encodeWithSignature("Panic(uint256)", 0x12);

    function getDiff(uint256 x, uint256 y)
        internal
        pure
    ...
    function assertIgnoringDecimals(
        uint256 x,
        uint256 y,
        uint256 decimalsToIgnore
    ) internal {
        assertEq(getDiff(x, y) / (10**decimalsToIgnore), 0);
    }

    function assertWithinPrecision(
        uint256 x,
        uint256 y,
        uint256 accuracy
    ) internal {
    ...
    function assertWithinPercentage(
        uint256 x,
        uint256 y,
        uint256 percentage
    ) internal {
    ...
    function convertUintToString(uint256 input_)
        internal
        pure
        returns (string memory output_)
    {
...
contract InvariantTest {
    ...
    {
        require(
            _excludedContracts.length != uint256(0),
            "NO_EXCLUDED_CONTRACTS"
        );
    ...
    {
        require(
            _targetedContracts.length != uint256(0),
            "NO_TARGETED_CONTRACTS"
        );
    ...
    {
        require(
            targetedSelectors_.length != uint256(0),
            "NO_TARGETED_SELECTORS"
        );
