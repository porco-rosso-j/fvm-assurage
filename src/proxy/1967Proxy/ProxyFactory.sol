...
abstract contract ProxyFactory {
    ...
    function _getImplementationOfProxy(address proxy_)
        private
        view
        returns (bool success_, address implementation_)
    {
    ...
    function _newInstance(uint256 version_, bytes memory arguments_)
        internal
        virtual
        returns (bool success_, address proxy_)
    {
        ...
        returns (bool success_, address proxy_)
    {
        ...
        success_ =
            (version != uint256(0)) &&
            _initializeInstance(proxy_, version, arguments_);
    }

    /// @dev Registers an implementation for some version.
    function _registerImplementation(uint256 version_, address implementation_)
        internal
        virtual
    ...
    function _registerMigrator(
        uint256 fromVersion_,
        uint256 toVersion_,
        address migrator_
    ) internal virtual returns (bool success_) {
        // Version 0 is invalid.
        if (fromVersion_ == uint256(0) || toVersion_ == uint256(0))
            return false;
    ...
    function _upgradeInstance(
        ...
        (success_, ) = proxy_.call(
            abi.encodeWithSelector(
                IProxied.setImplementation.selector,
                toImplementation
            )
        );
        ...
        address migrator = _migratorForPath[_versionOf[fromImplementation]][
            toVersion_
        ];
    ...
    function _getDeterministicProxyAddress(bytes32 salt_)
        ...
        view
        virtual
        returns (address deterministicProxyAddress_)
    {
    ...
    function _isContract(address account_)
        internal
        view
