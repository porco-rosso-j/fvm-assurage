...
contract AssurageGlobal is IAssurageGlobal, NonTransparentProxied {
    ...
    function activateAssurageManager(address _assurageManager) external override isGovernor {
        address delegate = IAssurageManager(_assurageManager).assurageDelegate();
        require(assurageDelegates[delegate].assurageManager == address(0), "MG:APM:ALREADYOWNS");
    ...
    function setMigrationAdmin(address _migrationAdmin) external override isGovernor {
    ...
    function setValidMiner(address miner, bool isValid) external override isGovernor {
        isMiner[miner] = isValid;
        emit ValidMinerSet(miner, isValid);
    }

    function setValidFactory(bytes32 factoryKey, address factory, bool isValid) external override isGovernor {
    ...
    function setValidAssurageDelegate(address account, bool isValid) external override isGovernor {
        ...
        require(isValid || assurageDelegates[account].assurageManager == address(0), "MG:SVPD:OWNSassurageManager");
    ...
    function setPlatformManagementFeeRate(address _assurageManager, uint256 _platformManagementFeeRate) external override isGovernor {
        require(_platformManagementFeeRate <= HUNDRED_PERCENT, "MG:SPMFR:RATEGT100");
        platformManagementFeeRate[_assurageManager] = _platformManagementFeeRate;
        emit PlatformManagementFeeRateSet(_assurageManager, _platformManagementFeeRate);
    }
    ...
    function isAssurageDelegate(address account_) external view override returns (bool _isAssurageDelegate) {
        _isAssurageDelegate = assurageDelegates[account_].isAssurageDelegate;
    }

    function assurageManager(address account_) external view override returns (address _assurageManager) {
