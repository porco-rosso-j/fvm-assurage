...
interface IAssurageManager is IAssurageProxied, IAssurageManagerStorage {
     ...
     */
    event VaultConfigured(uint256 minProtection, uint256 liquidityCap, uint256 delegateManagementFeeRate);
    ...
    event MinProtectionSet(uint minProtection);

    event BeneficiaryBytesAddrSet(bytes beneficiaryBytesAddr);

    event newApplicationMade(address miner, uint amount, uint period, uint id); 

    event newPolicyActivated(Policy policy, uint id);

    event newClaimFiled(Claim claim, uint id);

    event newCompensationPaid(Claim claim, uint id);
     ...
     */
    function configure(uint256 minProtection, uint256 liquidityCap, uint256 managementFee) external;
    ...
    function setMinProtection(uint _minProtection) external;

    function setAssessor(address _assessor) external;

    function setBeneficiaryBytesAddr(bytes memory _beneficiaryBytesAddr) external;

    function applyForProtection(address _miner, uint _amount, uint _period) external returns(Policy memory, uint);

    function activatePolicy(address _miner, uint _id) external returns(Policy memory);

    function fileClaim(address _miner, uint _id, uint _claimable) external;

    function payCompensation(address _miner, uint _id) external returns(uint);

    function renewPolocy(address _miner, uint _id) external returns(Policy memory);

     function approvePolicy(address _miner, uint _id, uint8 _score) external;

     function approveClaim(address _miner, uint _id, uint _claimable) external;

     function rejectClaim(address _miner, uint _id) external;

     function modifyScore(address _miner, uint _id, uint8 _score) external;

     function addStrategy(address _strategy) external;

     function investInStrategy(uint _index, uint _amount) external;

     function withdrawFromStrategy(uint _index, uint _amount) external;
