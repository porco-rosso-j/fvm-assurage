...
contract PoolDeployer is IVaultDeployer {
    ...
    function deployPool(
        ...
        uint[5] memory configParams // _minProtection, _liquidityCap, _delegateManagementFeeRate, _initialSupply
    )
        external override returns (address assurageManager)
    {
        ...
        require(_global.isAssurageDelegate(assurageDelegate), "PD:DP:INVALIDPD");
        require(_global.isFactory("POOLMANAGER", factories[0]), "PD:DP:INVALIDPMFACTORY");

        // Avoid stack too deep error
        {
            IAssurageProxyFactory PMFactory = IAssurageProxyFactory(factories[0]);

            require(
                initializers[0] == PMFactory.migratorForPath(PMFactory.defaultVersion(), PMFactory.defaultVersion()),
        ...
        bytes memory arguments = IAssurageManagerInitializer(
            initializers[0]).encodeArguments(assurageDelegate, asset, configParams[4], name, symbol
        );

        assurageManager = IAssurageProxyFactory(factories[0]).createInstance(arguments, salt);
        address vault = IAssurageManager(assurageManager).vault();

        // Configure Pool Manager
        IAssurageManager(assurageManager).configure(configParams[0], configParams[1], configParams[2]);
